...
compact 40-column fallback. The default wide double-height rendering requires
a font with Unicode 16 octants, even when separated mosaics use braille.
The braille default only avoids that requirement for separated mosaics.

Alignment assumes ambiguous-width Unicode characters occupy one terminal
column. Arrows, fractions, division, the double bar, the em dash and the solid
square are emitted with a
padding space in wide mode; terminals configured to display those characters
as two columns can shift the rest of the row. Use your terminal's single-column
setting for ambiguous characters. Narrow mode also depends on that setting.
